...
<script>
	...
	import TileController from "@/components/board/TileController.vue";
	import { mapGetters, mapActions } from "vuex";

	export default {
		components: { TileController },
		data() {
			return {
				sb: null,
		...
		methods: {
			...
			scale(number, fromLeft, fromRight, toLeft, toRight) {
				return (
					toLeft +
					((number - fromLeft) / (fromRight - fromLeft)) * (toRight - toLeft)
				);
			}
		},
		mounted() {
